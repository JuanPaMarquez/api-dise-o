<?php
/*
header("Access-Control-Allow-Origin: https://proyecto-biblioteca-num9991.000webhostapp.com/");
...
$servidor = "localhost"; $usuario = "id21803668_upxxe1minurxe31x"; $contrasenia = "gC6EVybmICcW7fmbGW6E*"; $nombreBaseDatos = "id21803668_bidkry6mrb47hhgehaiq";
...
// Insertar Historial
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_GET["insertarH"])) {
    $data = json_decode(file_get_contents("php://input"));

    if ($data && isset($data->idusuario, $data->idlibro, $data->titulo, $data->estado)) {
        $idusuario = mysqli_real_escape_string($conexionBD, $data->idusuario);
        $idlibro = mysqli_real_escape_string($conexionBD, $data->idlibro);
